...
double conc[ni];
    ...
    double xf=0.05+(0.5-0.05)*exp(-(x-N+1)*dxT*1e6);
   
    double K_FHAP=(7.39*exp(-10.7*xf)+3.3e-17)/1e32;
    ...
    double DS_HAP=pow(IP_HAP/K_FHAP,1/9.0);
    DSe[x]=DS_HAP;
...
if (DS_HAP>=1 & E_por[x]>0.01)
    ...
    E_por[x] += dt*M_HAP/rho_HAP*(rd_HAP[x]); //porosity change equation
    if(E_por[x]>1.0){
        E_por[x]=1.0;
    }
...
//  For Hydrohen ion
    conc[id_h]=conc_enamel[id_h][x] -(4.0-xf)*(rd_HAP[x])*dt;
    ...
    conc[id_f1n]=conc_enamel[id_f1n][x] +xf*(rd_HAP[x])*dt;
    if(conc[id_f1n] <0) {
        cerr<<"F";
        exit(EXIT_FAILURE);
        conc[id_f1n]=0.0;
    ...
    //For Ca ion
    conc[id_ca]=conc_enamel[id_ca][x] +5.0*(rd_HAP[x])*dt;
    if(conc[id_ca] <0){
    ...
    // For HPO4--  ion
    conc[id_pho]=conc_enamel[id_pho][x] +3*(rd_HAP[x])*dt;
    if(conc[id_pho] <0){
        //  cerr<<"pho2--";
        // exit(EXIT_FAILURE);
        conc[id_pho]=0.0;
