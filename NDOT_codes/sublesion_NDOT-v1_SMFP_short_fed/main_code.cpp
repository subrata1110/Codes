...
int main(){
    ...
    // charge nutral bulk 
                                ...
                                for (int j=0; j<n_ion-n_fixed_ions; j++){
                                    if(iobj[j].name != "k1p"){
                                        charge_density=charge_density+iobj[j].charge *conc_bulk_saliva[j];
                                    }
                                }
                                ...
                                conc_bulk_saliva[id_K1p]=-charge_density/iobj[id_K1p].charge;
...
<<","<<"rr_ca"<<"," <<"Porosity"<<","<<"DS"<<"," <<endl;
    ...
    // cout << conc_bulk_saliva[id_glu]<< " "<< conc_film[id_glu]<<endl;
        ...
        double fap_fraction=K_f*fl_frac*fl_frac*current_time/(1+K_f*fl_frac*current_time);
...
// cout<< it<<" k in enamel x=0 = "<< conc_enamel[id_k1p][0]<<endl;
    ...
    // Now nutralise charge by calcluating the potasium ions
        ...
        if(it%1200000   == 0 ){
            cout<<current_time/60.0 <<" "<< conc_film[id_glu] << " " << fap_fraction<<endl;
