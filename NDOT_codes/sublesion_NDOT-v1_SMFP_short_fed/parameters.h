...
const double K_HAP_enamel_F= 9.0*pow(10.0,-34.0); //solubility constant for FHAP surface mol^9/m^27 Ksp of FAP
...
///// Langmuir model qe=qbCe/(1+b*Ce)
const double conc_f=1.74; //fluoride concentration in mM or mol/m^3
const double fl_frac= 0.0035*502.31/19.0*(1.6*19)*conc_f/(1+(1.6*19)*conc_f); //molar fraction of the fluoride ion which replcaes the HAP
const double kd_F=pow(10,-4.9)/60.0;
const double K_f=0.91*19*1000/502.31/3600; // pseudo second order model for the kinetics

// Class for ions in the system
const string output_filename="all_ion_smfp_2p6mM_cagp_1p54.csv";
const string output_remin_demin="Remin_demin_smfp_2p6mMcagp_1p54.csv";
