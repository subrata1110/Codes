...
double conc[ni];
    ...
    int id_ca,id_h,id_pho, id, id_lach,id_pho1n, id_f1n;
...
//  For Hydrohen ion
    ...
    conc[id_f1n]=conc_enamel[id_f1n][x] +xf*(rd_HAP[x]-rr_HAP[x])*dt;
    if(conc[id_f1n] <0) {
        cerr<<"F";
        exit(EXIT_FAILURE);
        conc[id_f1n]=0.0;
...
void reaction_tcp(Ion* iobj,double conc[][N+NT-1],int ni ,int ix){
    int id_ca= id_ion(iobj,"ca2p",ni);
    int id_pho2n= id_ion(iobj,"pho2n",ni);
    int id_h= id_ion(iobj,"h1p",ni);
    int id_tcp=id_ion(iobj,"tcp",ni);
    double S=conc[id_ca][ix]*conc[id_ca][ix]*conc[id_ca][ix];
    S=S*conc[id_pho2n][ix]*conc[id_pho2n][ix]/conc[id_h][ix]/conc[id_h][ix]/K_sp_tcp;
    S=S*Ke_pho2n*Ke_pho2n;

    S=pow(S,1.0/5.0);
    
    // cout<<S<<" " <<endl;
    if(S<1.0){
        conc[id_ca][ix]=conc[id_ca][ix]+3*conc[id_tcp][ix];
        conc[id_pho2n][ix]=conc[id_pho2n][ix]+2.0*conc[id_tcp][ix];
        conc[id_tcp][ix]=0.0;
    }
...
int ni, int nrr, int x,  string inh_type[],  double K_inh[], int n_inh,double E_por[],double rd_HAP[],double rr_HAP[],
 double rd_ca[], double rr_ca[], double DSe[]){
    ...
    if(x>= 0 & x<N-1 ){ ///x=0,  x=NF-1 and x=NF+N-2 is the boundary
        ...
        reaction_tcp(iobj,conc,ni,x);

    } else {

        reaction_enamel(iobj,conc, ni-n_fixed_ions,x,E_por,rd_HAP,rr_HAP,rd_ca,rr_ca, DSe);
        reaction_tcp(iobj,conc,ni,x);
